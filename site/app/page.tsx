...
export default function Home() {
  ...
  // Mobile scroll-assist: snap hero panels after idle
  useEffect(() => {
    ...
    // Calculate how much of a panel is visible (0-1)
    const visibleFraction = (el: HTMLElement): number => {
    ...
    const snapToPanel = () => {
      ...
      const rightVis = visibleFraction(heroRight);
      const leftVis = visibleFraction(heroLeft);

      // Only snap if at least one panel is partially visible
      if (rightVis <= 0 && leftVis <= 0) return;

      let target: HTMLElement | null = null;

      if (rightVis >= 0.6) {
        // Yellow circle is dominant — snap to it
        target = heroRight;
      } else if (leftVis >= 0.6) {
        // White o1 lab section is dominant — snap to it
        target = heroLeft;
      } else if (rightVis > 0 && leftVis > 0) {
        // Both partially visible, neither dominant —
        // prefer the one we're scrolling toward
        target = scrollDirection === "down" ? heroLeft : heroRight;
      }
    ...
    const onScroll = () => {
      ...
      idleTimer = setTimeout(snapToPanel, 4000);
    };
